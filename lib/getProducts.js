...
import { withFallbackImage } from './productImageFallbacks';
  ...
  try {
    ...
    const transformedProducts = products.map(p => {
      ...
      return {
        ...
        metadata: p.metadata || {},
        features: safeJsonParse(p.metadata?.features),
        whatsIncluded: safeJsonParse(p.metadata?.whatsIncluded),
        faqs: safeJsonParse(p.metadata?.faqs),
        reviews: safeJsonParse(p.metadata?.reviews),
        reviewsRating: parseNumber(p.metadata?.reviewsRating),
        reviewsCount: parseNumber(p.metadata?.reviewsCount),
        msrp: parseNumber(p.metadata?.msrp),
